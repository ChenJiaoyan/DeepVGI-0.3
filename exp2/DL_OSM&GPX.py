...
def read_test_sample(n, test_imgs, p_imgs, n_imgs):
    ...
    for img in test_imgs:
        ...
        if [int(task_x), int(task_y)] in p_imgs:
            label[i, 1] = 1
            i += 1
            te_p += 1
        elif [int(task_x), int(task_y)] in n_imgs:
            label[i, 0] = 1
            i += 1
            te_n += 1
...
def read_train_sample(n1, n0, train_imgs, p_imgs, n_imgs):
    ...
    ne_imgs = random.sample(ne_imgs, n0)
    for i, img in enumerate(ne_imgs):
...
if __name__ == '__main__':
    ...
    print '--------------- Evaluation on OSM&GPX Samples ---------------'
    p_imgs, n_imgs = client.read_pn_images()
    img_X2, Y2 = read_test_sample(te_n, test_imgs, p_imgs, n_imgs)
